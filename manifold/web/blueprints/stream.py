"""Stream blueprint — passthrough proxy or FFmpeg HLS with filler overlays."""
...
@stream_bp.route("/<manifest_id>.m3u8")
def stream_playlist(manifest_id):
...
@stream_bp.route("/<manifest_id>/<path:segment>")
def stream_segment(manifest_id, segment):
    if not MANIFEST_ID_RE.match(manifest_id):
        abort(400)
    StreamManagerService.touch(manifest_id)
